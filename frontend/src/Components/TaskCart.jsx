    ...
    <div className="flex justify-between items-center bg-[#0F0F0F] hover:bg-[#3C3C3C] px-4 py-3 rounded-lg">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-white text-base font-semibold">{tsk.title || "Add New Task"}</p>
        </div>
      </div>
      ...
      <div className="flex items-center gap-3 text-gray-400">
        ...
        <X className="w-4 h-4 cursor-pointer hover:text-red-500" onClick={() => deleteHandler(tsk.id)}/>
      </div>
    </div>
  );
};
